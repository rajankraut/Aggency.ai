...
const Navbar = ({ theme, setTheme }) => {
  ...
  return (
    ...
    transition={{duration:0.6, ease:'easeOut'}}
     className="flex justify-between items-center px-4 sm:px-12 lg:px-24 xl:px-40 sticky top-0 z-20 backdrop-blur-xl font-medium bg-white/50 dark:bg-gray-900/70">
     
        <img src={theme=== "dark" ? assets.logo_dark : assets.logo} alt="Logo" className=" w-32 sm:w-40" />
      <div
        className={` text-gray-700 dark:text-white sm:text-m 
  flex sm:flex-row max-sm:flex-col sm:items-center gap-5 transition-all
  
     ${!sidebarOpen ?'max-sm:w-0':'max-sm:w-60 max-sm:pl-10' }
  max-sm:fixed max-sm:top-0 max-sm:right-0 
  max-sm:min-h-screen max-sm:h-full 
  max-sm:bg-primary max-sm:text-white max-sm:pt-20 max-sm:shadow-lg`}
      >
      ...
      <div className="flex items-center gap-2 sm:gap-4">
        ...
        <a
          href="#contact-us"
          className="text-m max-sm:hidden flex items-center gap-3 bg-primary text-white px-6 py-2 my-4 rounded-full cursor-pointer hover:scale-103 transition-all"
        >
